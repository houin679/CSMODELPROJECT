...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                ...
                distances[f'{j}'] = self.get_euclidean_distance(sliced_data, centroid) # columns take the values
            ...
            min_distances = distances.min(axis=1)  # gets the min distance for each data point
            index = min_distances.idxmax() # gets the greatest minimum distance
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if isinstance(point1, pd.Series) and isinstance(point2, pd.Series):
            ...
            euclidean_distance = np.sqrt(np.float64(squared_diff.sum()))
            return euclidean_distance

        # 1 DataFrame and 1 Series
        elif isinstance(point1, pd.DataFrame) and isinstance(point2, pd.Series):
            squared_diff = (point1.sub(point2, axis='columns')) ** 2
            euclidean_distance = np.sqrt(np.float64(squared_diff.sum(axis=1)))
            return euclidean_distance
    ...
    def group_observations(self, data):
        ...
        for i in range(self.k):
            ...
            distances[f'{i}'] = self.get_euclidean_distance(sliced_data, centroid)
    ...
    def adjust_centroids(self, data, groups):
        ...
        numeric_data = grouped_data.select_dtypes(include=np.number)
        centroids = numeric_data.groupby('group').mean()
